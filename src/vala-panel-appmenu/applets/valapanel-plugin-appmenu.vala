...
public class GlobalMenuApplet: Applet
{
    ...
    public override void constructed()
    {
        ...
        settings.bind(Key.ELLIPSIS_MODE,layout,Key.ELLIPSIS_MODE,SettingsBindFlags.DEFAULT);
    ...
    public override Widget get_settings_ui()
    {
        ...
        entry.show();
        entry = new CheckButton.with_label(dgettext(Config.GETTEXT_PACKAGE,"Use Ellipsis mode (overflow menus in ellipsis menu)"));
        this.settings.bind(Key.ELLIPSIS_MODE,entry,"active",SettingsBindFlags.DEFAULT);
        dlg.pack_start(entry,false,false,2);
        entry.show();
